...
import { ToDoStatus, TaskPriority } from '../../generated/prisma/index.js';
...
export const getTodo = async (req, res) => {
  try {
    ...
    const isManagerOfTargetUser = req.user.role === 'MANAGER' && 
      await isManagerOfUser(req.user.id, todo.userId);
...
export const createTodo = async (req, res) => {
  try {
    ...
    if (assignedUserId && assignedUserId !== req.user.id) {
      ...
      if (req.user.role !== 'MANAGER' && req.user.role !== 'ADMIN') {
        return res.status(403).json({ 
          message: 'Only managers can assign tasks to other users' 
        });
      }

      // Verify manager has authority over this user
      if (req.user.role === 'MANAGER') {
        const hasManagerAccess = await isManagerOfUser(
          req.user.id, 
          assignedUserId
        );
        if (!hasManagerAccess) {
          return res.status(403).json({ 
            message: 'You can only assign tasks to users you manage' 
          });
        }
      }
    ...
    // User creating task for themselves
      ...
      // Regular users need approval for self-created tasks
    ...
    // If requires approval, notify manager (implement email/notification here)
...
export const updateTodo = async (req, res) => {
  try {
    ...
    const isAdmin = req.user.role === 'ADMIN';
    const isManagerOfTargetUser = req.user.role === 'MANAGER' && 
      await isManagerOfUser(req.user.id, existingTodo.userId);
    ...
    if (status === ToDoStatus.PENDING && (isAssigner || isAdmin || isManagerOfTargetUser) && 
        existingTodo.isSelfCreated) {
      if (existingTodo.status === ToDoStatus.PENDING_APPROVAL) {
...
export const getTodoStats = async (req, res) => {
  try {
    ...
    if (targetUserId !== req.user.id) {
      const isAdmin = req.user.role === 'ADMIN';
      const hasManagerAccess = req.user.role === 'MANAGER' && 
        await isManagerOfUser(req.user.id, targetUserId);
    ...
    const completedTasks = allTasks.filter(t => 
      t.status === ToDoStatus.COMPLETED && t.completedAt
    );
    ...
    if (completedTasks.length > 0) {
      ...
      stats.completion.averageCompletionTime = (
        totalCompletionTime / completedTasks.length / (1000 * 3600)
      ).toFixed(2);
    ...
    stats.mostProductiveDays = dayNames
      .map((day, index) => ({
        day,
...
export const approveSelfCreatedTask = async (req, res) => {
  try {
    ...
    if (!hasManagerAccess && req.user.role !== 'ADMIN') {
      return res.status(403).json({ 
        message: 'Only the manager can approve self-created tasks' 
      });
    }
    ...
    const updatedTodo = await prisma.toDo.update({
      ...
      data: {
        ...
        rejectionReason: !approved 
          ? (req.body.rejectionReason || 'Task not approved') 
          : null,
