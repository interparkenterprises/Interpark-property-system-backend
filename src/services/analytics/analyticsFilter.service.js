...
export const normalizeAnalyticsFilters = (query = {}, now = new Date()) => {
  ...
  // Allow dateFrom without dateTo and vice versa - treat as "all time" if not provided
  // But if both are provided, they must be valid
  let dateFrom, dateTo, start, endExclusive;
  let hasDateFilter = false;

  if (hasFrom && hasTo) {
    // Both provided - validate and use them
    dateFrom = query.dateFrom;
    dateTo = query.dateTo;
    start = assertDate(dateFrom, 'dateFrom');
    assertDate(dateTo, 'dateTo');
    endExclusive = assertDate(addLocalDays(dateTo, 1), 'dateTo');
    ...
    // Only dateFrom provided - use it as start, no end limit
    dateFrom = query.dateFrom;
    start = assertDate(dateFrom, 'dateFrom');
    hasDateFilter = true;
  } else if (!hasFrom && hasTo) {
    // Only dateTo provided - use it as end, no start limit
    dateTo = query.dateTo;
    assertDate(dateTo, 'dateTo');
    endExclusive = assertDate(addLocalDays(dateTo, 1), 'dateTo');
    hasDateFilter = true;
  } else {
    // Neither provided - use default (current month)
    dateFrom = firstDayOfMonth(today);
    dateTo = today;
    start = assertDate(dateFrom, 'dateFrom');
    assertDate(dateTo, 'dateTo');
    endExclusive = assertDate(addLocalDays(dateTo, 1), 'dateTo');
    hasDateFilter = true; // Still apply default date filter for most analytics
  }

  const asOf = query.asOf || dateTo || today;
  ...
  return {
    ...
    asOfExclusive,
    timezone: TIMEZONE,
    hasDateFilter // Flag to indicate if date filter should be applied
  };
};
